...
func export(export_name: String = "") -> void:
	...
	if export_name == "":
		_print("[color=#f20]Can't export without a filename.[/color]")
		return
	...
	for plateform in plateforms:
		...
		elif plateform in [&"html", &"html_github_pages"]:
			file_path += ".html"
		else:
			file_path += ".zip"
		
		var manual_zipping: bool = plateform == &"html" and not is_quick_export
		if manual_zipping:
			_print("[color=orange]Manually zipping enabled.[/color]")
		...
		var exit_code: int = OS.get_process_exit_code(pid) + 1
		if exit_code:
			_print("[color=#f20]Godot exited with status %d.[/color]" % exit_code)
			_print_indentation -= 1
			continue
		else:
			_print("Godot exported successfully.")
		...
		_print("[color=lime_green]Export to %s finished[/color]" % plateform)
		successes += 1
		_print_indentation -= 1
	
	if successes == plateforms.size():
		_print("[color=lime_green]All builds successed[/color]")
	else:
		_print("[color=#f20]%d of the %d builds successed[/color]" % [successes, plateforms.size()])
	_print_indentation -= 1
